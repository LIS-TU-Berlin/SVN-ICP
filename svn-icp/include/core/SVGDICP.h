...
/**
 * @file    SVGDICP.h
 * @brief   Basic class of revised SVGD ICP
 * @author  UNKNOWN
 * @author  UNKNOWN
 * @date    June 22, 2025
 */
...
namespace svnicp {
  ...
  public: // functions
    ...
    explicit SVGDICP(const SteinICPParam &parameters, const torch::Tensor &init_pose);

    virtual ~SVGDICP() = default;
    ...
    * \return pose particles, size: particle_count*6*/
    ...
    std::vector<double> get_particles() const;
    ...
    std::vector<std::vector<float> > get_particle_history() const;
    ...
    void set_k(const int k) { K_source_ = k; };

    void set_threshold(const double max_dist) { config_.max_dist = max_dist; };

    void set_initial_mean(const gtsam::Pose3 &pose) {
      const auto R0 = torch::from_blob(pose.rotation().matrix().data(), {3, 3},
                                       torch::TensorOptions().dtype(data_type)).to(torch::kCUDA);

      const auto t0 = torch::from_blob((void *) pose.translation().data(), {3, 1},
                                       torch::TensorOptions().dtype(data_type)).to(torch::kCUDA);
      R0_ = R0.transpose(0, 1);
      t0_ = t0;
    };
  ...
  protected: // functions
    ...
    static torch::Tensor to_translation_tensor(const torch::Tensor &x, const torch::Tensor &y, const torch::Tensor &z);
