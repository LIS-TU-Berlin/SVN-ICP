...
#include "core/SVGDICP.h"
#include "core/SVNICP.h"

namespace svnicp {
  OdometryPipeline::OdometryPipeline() : rclcpp::Node("SVNICP") {
    // parameters
    this->load_param();
    ...
    if (estimator_ == ICP) {
      ...
      updater_ = [&](const gtsam::Pose3 &initial_guess,
        ...
        const auto correction_pose = svnicp::tensor2gtsamPose3(correction_tr);
    ...
    imu_subscriber_ = this->create_subscription<sensor_msgs::msg::Imu>(imu_topic_,
                                                                       rclcpp::SensorDataQoS().reliability(
    ...
    // publishers
    path_publisher_ = this->create_publisher<nav_msgs::msg::Path>("/svnicp/trajectories", 10);
    state_publisher_ = this->create_publisher<nav_msgs::msg::Odometry>("/svnicp/body_state", 10);
    prediction_publisher_ = this->create_publisher<geometry_msgs::msg::PoseWithCovarianceStamped>(
      "/svnicp/prediction", 10);
    raw_cloud_publisher_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("/svnicp/raw_cloud", 1);
    source_publisher_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("/svnicp/source_cloud", 1);
    localmap_publisher_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("/svnicp/localmap_cloud", 1);
    neighbourmap_publisher_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("/svnicp/neighbourmap_cloud", 1);
    last_particle_publisher_ = this->create_publisher<stein_msgs::msg::SteinParticle>("/svnicp/particles", 10);
    stein_param_publisher_ = this->create_publisher<stein_msgs::msg::SteinParameters>("/svnicp/parameters", 10);
    runtime_publisher_ = this->create_publisher<stein_msgs::msg::Runtime>("/svnicp/runtime", 10);
    all_particle_publisher_ = this->create_publisher<stein_msgs::msg::SteinParticleArray>(
      "/svnicp/all_particles", 10);
    variance_publisher_ = this->create_publisher<stein_msgs::msg::Variance>("/svnicp/variance", 10);
    KF_Gain_publisher_ = this->create_publisher<stein_msgs::msg::SteinParticle>("/svnicp/kf_gain", 10);
  ...
  void OdometryPipeline::load_param() {
    //SteinICP parameters
    std::string estimator_class;
    this->declare_parameter("estimator", "KF");
    this->get_parameter("estimator", estimator_class);
    if (estimator_class == "KF") estimator_ = KF;
    else estimator_ = ICP;

    this->declare_parameter("class_type", "SVNICP");
    this->get_parameter("class_type", class_type_);
    this->declare_parameter("SVNICP_parameters.optimizer", "Adam");
    this->get_parameter("SVNICP_parameters.optimizer", steinicp_config_.optimizer);
    this->declare_parameter("SVNICP_parameters.iterations", 50);
    this->get_parameter("SVNICP_parameters.iterations", steinicp_config_.iterations);
    this->declare_parameter("SVNICP_parameters.batch_size", 50);
    this->get_parameter("SVNICP_parameters.batch_size", steinicp_config_.batch_size);
    this->declare_parameter("SVNICP_parameters.normalize", true);
    this->get_parameter("SVNICP_parameters.normalize", steinicp_config_.normalize_cloud);
    this->declare_parameter("SVNICP_parameters.lr", 0.02);
    this->get_parameter("SVNICP_parameters.lr", steinicp_config_.lr);
    this->declare_parameter("SVNICP_parameters.max_dist", 2.8);
    this->get_parameter("SVNICP_parameters.max_dist", steinicp_config_.max_dist);
    this->declare_parameter("SVNICP_parameters.knn_count", 100);
    this->get_parameter("SVNICP_parameters.knn_count", steinicp_config_.KNN_count);
    this->declare_parameter("SVNICP_parameters.particle_size", 100);
    this->get_parameter("SVNICP_parameters.particle_size", particle_count_);
    this->declare_parameter("SVNICP_parameters.frame_gap_seconds", 0.35);
    this->get_parameter("SVNICP_parameters.frame_gap_seconds", msg_buffer_gap_);
    this->declare_parameter("SVNICP_parameters.using_EarlyStop", false);
    this->get_parameter("SVNICP_parameters.using_EarlyStop", steinicp_config_.check_early_stop);
    this->declare_parameter("SVNICP_parameters.convergence_steps", 5);
    this->get_parameter("SVNICP_parameters.convergence_steps", steinicp_config_.convergence_steps);
    this->declare_parameter("SVNICP_parameters.convergence_threshold", 1e-5);
    this->get_parameter("SVNICP_parameters.convergence_threshold", steinicp_config_.convergence_threshold);
    this->declare_parameter("lidar_param.topic", "/velodyne_points");
    this->get_parameter("lidar_param.topic", cloud_topic_);
    this->declare_parameter("SVNICP_parameters.deskew_cloud", false);
    this->get_parameter("SVNICP_parameters.deskew_cloud", deskew_cloud_);
    this->declare_parameter("SVNICP_parameters.save_particles", false);
    this->get_parameter("SVNICP_parameters.save_particles", save_particles_);

    this->declare_parameter("SVNICP_parameters.max_range", 80.0);
    this->get_parameter("SVNICP_parameters.max_range", max_range_);
    this->declare_parameter("SVNICP_parameters.min_range", 0.0);
    this->get_parameter("SVNICP_parameters.min_range", min_range_);

    this->declare_parameter("SVNICP_parameters.pub_cloud", false);
    this->get_parameter("SVNICP_parameters.pub_cloud", pub_cloud_);
    this->declare_parameter("SVNICP_parameters.map_voxel_max_points", 10);
    this->get_parameter("SVNICP_parameters.map_voxel_max_points", local_map_.max_pointscount_);

    this->declare_parameter("SVNICP_parameters.map_range", 100.);
    this->get_parameter("SVNICP_parameters.map_range", local_map_.max_range_);

    this->declare_parameter("SVNICP_parameters.map_voxel_size", 1.0);
    this->get_parameter("SVNICP_parameters.map_voxel_size", local_map_.voxel_size_);


    this->declare_parameter("SVNICP_parameters.voxel_size", 0.01);
    this->get_parameter("SVNICP_parameters.voxel_size", voxel_size_);

    this->declare_parameter("SVNICP_parameters.USE_Segmentation", true);
    this->get_parameter("SVNICP_parameters.USE_Segmentation", use_Segmentation_);
    if (cloud_topic_ == "/rslidar_points")
      use_Segmentation_ = false;
    this->declare_parameter("SVNICP_parameters.use_weight_mean", false);
    this->get_parameter("SVNICP_parameters.use_weight_mean", stein_micp_opt_.use_weight_mean);


    std::string cov_filter_type;
    this->declare_parameter("SVNICP_parameters.cov_filter_type", "none");
    this->get_parameter("SVNICP_parameters.cov_filter_type", cov_filter_type);
    ...
    this->declare_parameter("SVNICP_parameters.SVNFullGrad", false);
    this->get_parameter("SVNICP_parameters.SVNFullGrad", steinicp_config_.SVN_full_grad);

    // sensor parameter
    this->declare_parameter("imu_param.topic", "/sensor/data");
  ...
  void OdometryPipeline::allocateMemory() {
    this->set_initPose();
    if (estimator_ == KF)
      kalman_filter_ = std::make_shared<estimator::ErrorStateKalmanFilter>(lio_param_, imu_noise_);
    if (class_type_ == "SVNICP") {
      std::cout << "Lidar odometry with SVN-ICP " << std::endl;
      steinicp_odom_ = std::make_unique<svnicp::SVNICP>(steinicp_config_, init_pose_, stein_micp_opt_);
    } else {
      std::cout << "Lidar odometry with SVGD-ICP " << std::endl;
      steinicp_odom_ = std::make_unique<svnicp::SVGDICP>(steinicp_config_, init_pose_);
    }
  ...
  void OdometryPipeline::imu_msg_cb(const sensor_msgs::msg::Imu::SharedPtr &msg) {
    svnicp::data_types::IMUMeasurement imu_measurement;
  ...
  void OdometryPipeline::lidar_msg_cb(const sensor_msgs::msg::PointCloud2::SharedPtr &msg) {
    ...
    auto segmented_msg = std::make_shared<sensor_msgs::msg::PointCloud2>();
  ...
  pcl::PointCloud<Point_t> OdometryPipeline::deskew_pointcloud(const sensor_msgs::msg::PointCloud2::SharedPtr &msg) {
    ...
    if (cloud_topic_ == "/kitti/velo/pointcloud") {
      ...
      tbb::parallel_for(size_t(0), frame->size(), [&](size_t i) {
        const Eigen::Vector3d pt(frame_points[i].x, frame_points[i].y, frame_points[i].z);
        ...
        const auto x = frame_points[i].x;
        const auto y = frame_points[i].y;
    ...
    const auto [min_time_it, max_time_it] = std::minmax_element(extracted_timestamps.begin(),
                                                                extracted_timestamps.end());
    ...
    const auto pose_number = poses_buffer.size();
    const auto start_frame = poses_buffer.get_buffer_from_id(pose_number - 2);
    const auto start_time = poses_buffer.time_buffer[pose_number - 2];
    const auto finish_frame = poses_buffer.get_last_buffer();
    ...
    const auto delta_pose = gtsam::Pose3::Logmap(gtsam::Pose3(start_frame.matrix().inverse() * finish_frame.matrix()));
    ...
    tbb::parallel_for(size_t(0), point_number, [&](size_t i) {
      ...
      auto deskewed_point = motion.
          transformFrom(gtsam::Point3(frame_points[i].x, frame_points[i].y, frame_points[i].z));
  ...
  void OdometryPipeline::ICP_processing() {
    ...
    svnicp::data_types::IMUMeasurement imu_previous, imu_current;
    ...
    while (rclcpp::ok()) {
      // sync IMU and LiDAR
      if (imu_measurement_buffer_.size() != 0 && estimator_ == KF) {
        ...
        if (cloud_msg_buffer_.size() == 0 && synced) {
          ...
          if (auto first_lidar_time = cloud_msg_buffer_.get_first_time(); abs(first_lidar_time.seconds() - imu_time.seconds()) < 0.001) {
            body_state_pre_ = body_state_;
            kalman_filter_->predict(body_state_, imu_previous, imu_current);
            imu_previous = imu_current;
          }
          // LiDAR timestamp is slower than sensor
          else if (first_lidar_time.seconds() < imu_time.seconds()) {
            // LiDAR between two sensor, interpolate IMU
            if (first_lidar_time.seconds() > imu_previous.timestamp.seconds()) {
              auto interpolate_imu = sensor::IMUInterpolation(first_lidar_time, imu_previous, imu_current);
          ...
          const auto [origin_msg, timestamp] = cloud_msg_buffer_.get_first_buffer_time_pair_and_pop();
      ...
      } else if (estimator_ == ICP && cloud_msg_buffer_.size() != 0) {
        const auto [origin_msg, timestamp] = cloud_msg_buffer_.get_first_buffer_time_pair_and_pop();
        //                    auto dummy  = imu_measurement_buffer_.get_first_buffer_time_pair_and_pop();
        source_msg = origin_msg;
        lidar_time = timestamp;
      ...
      *source_pcl_ = *voxelized_cloud;
      ...
      this->set_initPose();
      source_tr_ = svnicp::vector2tensor(svnicp::pointcloud2vector(source_pcl_), torch::kCUDA);
      // get map
      if (!local_map_.Empty()) {
        *target_pcl_ = local_map_.GetMap(initial_guess, scan_max_range_ + 10.);
        if (target_pcl_->empty()) {
          *target_pcl_ = local_map_.GetMap();
        }
        target_tr_ = svnicp::vector2tensor(svnicp::pointcloud2vector(target_pcl_), torch::kCUDA);
      ...
      if (svnicp::SteinICPState align_state = steinicp_odom_->stein_align(); align_state != SteinICPState::ALIGN_SUCCESS) {
        continue;
      }
      const auto correction_tr = steinicp_odom_->get_transformation(); // the output pose, size 6*1
      const auto cov_tr = steinicp_odom_->get_distribution(); // variance, size 6*1
      const auto cov_mat = steinicp_odom_->get_cov_matrix();

      const auto stein_particles = steinicp_odom_->get_particles(); // particles in the last iteration
      const auto stein_weights = steinicp_odom_->get_particle_weight(); // weights
      ...
      //            if(estimator==ICP){
      //                auto correction_pose = core::tensor2gtsamPose3(correction_tr);
      //                pose3_current =  gtsam::Pose3(initial_guess.matrix()*correction_pose.matrix());
      //            }
      //            else {
      //                this->kf_updater(pose3_current, correction_tr, var_vector, cov_mat, lidar_time);
      ...
      if (dataset_duration_ != 0) {
        const auto current_time = rclcpp::Time(source_msg->header.stamp.sec, source_msg->header.stamp.nanosec,
                                               RCL_ROS_TIME);
        const auto duration = current_time - first_time;
        std::cout << "Current Progress: " << double(duration.nanoseconds()) / dataset_duration_ * 100. << "%" <<
            std::endl;
      }
    }
  }
  ...
  void OdometryPipeline::set_initPose() {
    const torch::Tensor lb = torch::tensor({-0.3, -0.2, -0.1, -0.004, -0.004, -0.012},
                                           torch::TensorOptions().dtype(torch::kFloat64).device(torch::kCUDA));
    const torch::Tensor ub = torch::tensor({0.3, 0.2, 0.1, 0.004, 0.004, 0.012},
                                           torch::TensorOptions().dtype(torch::kFloat64).device(torch::kCUDA));
    init_pose_ = svnicp::initialize_particles(particle_count_, torch::kCUDA, ub, lb);
  }

  // initializa as a Gaussian distribution
  void OdometryPipeline::set_initPose(const gtsam::Vector6 &cov) {
    init_pose_ = svnicp::initialize_particles_gaussian(particle_count_, torch::kCUDA, cov);
  }

  // downsample using pcl voxel, mean point in each voxel
  pcl::PointCloud<Point_t> OdometryPipeline::downsample_voxel(const pcl::PointCloud<Point_t>::Ptr &cloud,
                                                              double voxel_size) {
  ...
  pcl::PointCloud<Point_t> OdometryPipeline::downsample_uniform(const pcl::PointCloud<Point_t>::Ptr &cloud,
                                                                double voxel_size) {
  ...
  pcl::PointCloud<Point_t> OdometryPipeline::crop_pointcloud(
  ...
  gtsam::Pose3 OdometryPipeline::pose_prediction(const rclcpp::Time &new_time) {
    ...
    const auto start_frame = poses_buffer.get_buffer_from_id(N - 2);
    const auto start_time = poses_buffer.time_buffer[N - 2];
    const auto finish_frame = poses_buffer.get_last_buffer();
    const auto finish_time = poses_buffer.get_last_time();
    const auto delta_time = finish_time.seconds() - start_time.seconds();
    const auto new_delta_time = new_time.seconds() - finish_time.seconds();

    // dT = inv(T_k-2) * T_k-1
    delta_pose = gtsam::Pose3(start_frame.matrix().inverse() * finish_frame.matrix());
    const auto time_ratio = new_delta_time / delta_time;
    delta_pose = gtsam::Pose3::Expmap(time_ratio * gtsam::Pose3::Logmap(delta_pose));

    // T_pred = T_k-1 * dT
    const auto initial_guess = gtsam::Pose3(finish_frame.matrix() * delta_pose.matrix());

    body_state_.state = gtsam::NavState(initial_guess,
                                        gtsam::Pose3::Expmap(gtsam::Pose3::Logmap(delta_pose) / delta_time).
                                        translation());
    return initial_guess;
  }

  void OdometryPipeline::kf_updater(gtsam::Pose3 &pose3_current,
                                    const torch::Tensor &correction_tr,
                                    gtsam::Vector6 &var_vector,
                                    const std::vector<double> &cov_mat,
    ...
    gtsam::Matrix6 cov_mat_diag = var_vector.asDiagonal();
    const gtsam::Vector6 icp_correction = svnicp::tensor2Vector6(correction_tr);
    for (size_t i = 0; i < 6; ++i)
      var_vector[i] *= icp_cov_scales_[i];

    gtsam::Matrix66 cov_smooth;
    this->cov_smoother(cov_smooth, 20);
    stein_msgs::msg::Variance var_msg;
    ...
    if (use_constCov_ || particle_count_ == 1)
      cov_mat_diag = gtsam::Vector6(constCov_[0], constCov_[0], constCov_[0], constCov_[1], constCov_[1], constCov_[1]).
          asDiagonal();
    ...
    stein_msgs::msg::SteinParticle kf_gain_msg;
  ...
  void OdometryPipeline::cov_smoother(gtsam::Matrix66 &cov, double window_size) {
    const int N = cov_matrix_buffer_.size();
    if (N == 0)
      cov = gtsam::Vector6(constCov_[0], constCov_[0], constCov_[0], constCov_[1], constCov_[1], constCov_[1])
          .asDiagonal();
    if (N < window_size) window_size = N;
    gtsam::Matrix66 sum = gtsam::Matrix66::Zero();
    for (int i = 0; i < window_size; i++) {
      const auto cov_pre = cov_matrix_buffer_.get_buffer_from_id(N - i - 1);
      sum += cov_pre;
    }
    cov = sum / window_size;
  }

  void OdometryPipeline::variance_prediction() {
    ...
    if (N < 2)
      ...
      const auto cov_begin = cov_matrix_buffer_.get_buffer_from_id(N - 2);
      const auto cov_end = cov_matrix_buffer_.get_buffer_from_id(N - 1);
      gtsam::Vector6 cov = (cov_begin + cov_end).diagonal();

      this->set_initPose(cov);

      const double delta = sqrt((cov.x() + cov.y() + cov.z())) + 2.0 * max_range_ * std::sin(
                       sqrt(cov[3] + cov[4] + cov[5]) / 2.0);
  ...
  void OdometryPipeline::publish_stein_param() {
    stein_msgs::msg::SteinParameters stein_param_msg;
  ...
  void OdometryPipeline::publish_odometry() {
    ...
    path_msg_.header.frame_id = "odom_svnicp";
    ...
    prediction_msg.header.frame_id = "odom_svnicp";
    ...
    transformStamped_msg.child_frame_id = "odom_svnicp";
  ...
  void OdometryPipeline::publish_cloud(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &publisher,
                                       std::vector<std::pair<rclcpp::Time, pcl::PointCloud<Point_t> > > &cloud_vec) const {
    pcl::PointCloud<Point_t> cloud_pcl;
    for (const auto &[fst, snd]: cloud_vec) {
      this->publish_cloud(publisher, snd, fst);
    }
    cloud_vec.clear();
  }
  ...
  void OdometryPipeline::publish_cloud(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &publisher,
    ...
    cloud_msg.header.frame_id = "odom_svnicp";
    cloud_msg.header.stamp = timestamp;
    publisher->publish(cloud_msg);
  }

  void OdometryPipeline::publish_particle_info() {
    if (stein_particle_weight_buffer_.size() > 0) {
      stein_msgs::msg::SteinParticle particle_msg;
      const auto particle_weight_to_pub = stein_particle_weight_buffer_.get_last_buffer();
      const auto timestamp = stein_particle_weight_buffer_.get_last_time();
      const auto particles = particle_weight_to_pub.first;
      const auto weights = particle_weight_to_pub.second;
  ...
  void OdometryPipeline::publish_all_particles(const std::vector<std::vector<float> > &particles) const {
    stein_msgs::msg::SteinParticleArray all_particle_msg;
    all_particle_msg.header.frame_id = "odom_svnicp";
    for (auto particle_per_loop: particles) {
      ...
      particle_msg.y = std::vector<double>(particle_per_loop.begin() + particle_count_,
                                           particle_per_loop.begin() + 2 * particle_count_);
      particle_msg.z = std::vector<double>(particle_per_loop.begin() + 2 * particle_count_,
                                           particle_per_loop.begin() + 3 * particle_count_);
      particle_msg.roll = std::vector<double>(particle_per_loop.begin() + 3 * particle_count_,
                                              particle_per_loop.begin() + 4 * particle_count_);
      particle_msg.pitch = std::vector<double>(particle_per_loop.begin() + 4 * particle_count_,
                                               particle_per_loop.begin() + 5 * particle_count_);
      particle_msg.yaw = std::vector<double>(particle_per_loop.begin() + 5 * particle_count_,
                                             particle_per_loop.begin() + 6 * particle_count_);
      all_particle_msg.stein_particle_array.push_back(particle_msg);
    }
    all_particle_publisher_->publish(all_particle_msg);
  }

  void OdometryPipeline::publish_runtime() const {
    stein_msgs::msg::Runtime runtime_msg;
  ...
  void OdometryPipeline::publish_thread_cb() {
