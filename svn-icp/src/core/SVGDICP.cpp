...
namespace svnicp {
  SVGDICP::SVGDICP(const SteinICPParam &parameters, const torch::Tensor &init_pose) {
    ...
    pose_particles_ = torch::stack({
      x_ * normalize_factor_, y_ * normalize_factor_, z_ * normalize_factor_, rx_, ry_, rz_
    }).squeeze({2, 3});
    R_ = SVGDICP::to_rotation_tensor(rx_, ry_, rz_); //p*3*3
  ...
  void SVGDICP::add_cloud(const torch::Tensor &source, const torch::Tensor &target, const torch::Tensor &init_pose) {
    particle_size_ = init_pose.size(1);
    x_ = init_pose[0].view({particle_size_, 1, 1});
    y_ = init_pose[1].view({particle_size_, 1, 1});
    z_ = init_pose[2].view({particle_size_, 1, 1});
  ...
  /*Stein ICP process*/
  SteinICPState SVGDICP::stein_align() {
    knn_duration_ = 0;
    update_duration_ = 0;
    ...
    for (int epoch = 0; epoch < config_.iterations; epoch++) {
      ...
      torch::Tensor mini_batch_epoch = mini_batch_all_epochs[epoch].expand({particle_size_, config_.batch_size, 3}).
          to(torch::kCUDA).to(data_type);
      ...
      torch::Tensor source_transformed = mini_batch_epoch.matmul(R_total_.transpose(1, 2)) + t_total_.view({
                                           particle_size_, 1, 3
                                         });
  ...
  void SVGDICP::allocate_memory_for_log() {
    particle_stack_ = torch::zeros({config_.iterations, 6, particle_size_}).to(torch::kCUDA);
  }

  std::tuple<torch::Tensor, torch::Tensor> SVGDICP::mini_batch_pair_generator() {
    ...
    //        std::cout << "idx \n" << idx << std::endl;
    torch::Tensor mini_batch = torch::empty({config_.batch_size * config_.iterations, 3}).to(torch::kCUDA).
        to(data_type);
    ...
    return {
      mini_batch.reshape({config_.iterations, config_.batch_size, 3}),
      target_batch.reshape({config_.iterations, config_.batch_size, K_source_, 3})
    };
  }

  void SVGDICP::knn_source_cloud() {
  ...
  torch::Tensor SVGDICP::mini_batch_generator() {
  ...
  torch::Tensor SVGDICP::to_rotation_tensor(const torch::Tensor &r, const torch::Tensor &p, const torch::Tensor &y) {
  ...
  torch::Tensor SVGDICP::to_translation_tensor(const torch::Tensor &x, const torch::Tensor &y, const torch::Tensor &z) {
    return torch::cat({x, y, z}, 1);
  }

  torch::Tensor SVGDICP::rotm_to_ypr_tensor(const torch::Tensor &R) {
  ...
  std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> SVGDICP::get_correspondence(
    ...
    const auto knn = KNearestNeighborIdx(transformed_source,
                                         target.expand({particle_size_, target.size(0), 3}).to(torch::kCUDA).to(
                                           data_type),
                                         config_.batch_size * torch::ones_like(x_).reshape({particle_size_, 1}).to(
                                           torch::kLong),
                                         target.size(0) * torch::ones_like(x_).reshape({particle_size_, 1}).to(
                                           torch::kLong),
                                         2,
                                         1,
                                         -1);
  ...
  std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> SVGDICP::get_correspondence_fast(
    const torch::Tensor &source, /// particle*config_.batch_size * 3
    const torch::Tensor &transformed_source, /// particle*config_.batch_size * 3
    const torch::Tensor &target /// target_point_count * 3
  ) {
    const auto knn = KNearestNeighborIdx(transformed_source.transpose(0, 1),
                                         target,
                                         particle_size_ * torch::ones({config_.batch_size, 1}).to(torch::kLong).to(
                                           torch::kCUDA),
                                         K_source_ * torch::ones({config_.batch_size, 1}).to(torch::kLong).to(
                                           torch::kCUDA),
                                         2,
                                         1,
                                         -1);
    const auto dist_cr = std::get<1>(knn).transpose(0, 1).to(torch::kCUDA);
    const auto target_index = std::get<0>(knn).transpose(0, 1).to(torch::kCUDA).reshape({
      particle_size_, config_.batch_size
    });

    return {
      point_filter(source, dist_cr),
      point_filter(transformed_source, dist_cr),
      point_filter((target.transpose(0, 1)).index({
                     target_index,
  ...
  torch::Tensor SVGDICP::point_filter(const torch::Tensor &cloud, const torch::Tensor &distance_cr) {
    return torch::lt(distance_cr, config_.max_dist / normalize_factor_).to(torch::kCUDA) * cloud;
  }

  torch::Tensor SVGDICP::partial_derivative(const torch::Tensor &roll,
    ...
    const torch::Tensor partial_roll = torch::stack(
      {
        torch::stack({
                       torch_0, Cosyaw_Sinpitch_Cosroll + Sinyaw_Sinroll, Sinyaw_Cosroll - Cosyaw_Sinpitch_Sinroll
                     }, 3).squeeze(1),
        torch::stack(
          {
            torch_0, -Cosyaw_Sinroll + Sinyaw_Sinpitch_Cosroll, Sinyaw * (-Sinpitch_Sinroll) - Cosyaw_Cosroll
          }, 3).squeeze(1),
        torch::stack({torch_0, Cospitch * Cosroll, Cospitch * (-Sinroll)}, 3).squeeze(1)
      },
      2).squeeze(1);

    const torch::Tensor partial_pitch = torch::stack(
      {
    ...
    const torch::Tensor partial_yaw = torch::stack(
      {
        torch::stack({
                       -Sinyaw_Cospitch, -Sinyaw * Sinpitch_Sinroll - Cosyaw_Cosroll,
                       Cosyaw_Sinroll - Sinyaw_Sinpitch_Cosroll
                     }, 3).squeeze(1),
        torch::stack({
                       Cosyaw_Cospitch, -Sinyaw_Cosroll + Cosyaw_Sinpitch_Sinroll,
                       Cosyaw_Sinpitch_Cosroll + Sinyaw_Sinroll
                     }, 3).squeeze(1),
  ...
  torch::Tensor SVGDICP::sgd_grad(const torch::Tensor &source_paired,
    ...
    constexpr c10::optional<int64_t> dim = 1;

    const auto nonzero_count = (torch::count_nonzero(transformed_s_paired.sum(2), dim).to(torch::kCUDA)).to(data_type);
    ///particle*1
    ...
    sgd_gradient.index_put_({Slice(), 3}, torch::einsum("pbc, pbc->pb",
                                          ...
                                          (nonzero_count + torch::ones_like(nonzero_count).to(torch::kCUDA))) /
        normalize_factor_;
    // gradient of pitch
    sgd_gradient.index_put_({Slice(), 4}, torch::einsum("pbc, pbc->pb",
                                          ...
                                          (nonzero_count + torch::ones_like(nonzero_count).to(torch::kCUDA).to(
                                             data_type))) / normalize_factor_;
    // gradient of yaw
  ...
  torch::Tensor SVGDICP::svgd_grad(const torch::Tensor &pose_parameters, const torch::Tensor &sgd_grad) {
  ...
  std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> SVGDICP::rbf_kernel(const torch::Tensor &x) {
  ...
  void SVGDICP::pose_update(const torch::Tensor &stein_grad) {
  ...
  torch::Tensor SVGDICP::get_transformation() {
    return torch::mean(pose_particles_, 1);
  }

  torch::Tensor SVGDICP::get_distribution() {
    return torch::var(pose_particles_, 1);
  }

  std::vector<double> SVGDICP::get_cov_matrix() {
    const auto mean = this->get_transformation();
    const auto difference = pose_particles_ - mean.view({6, 1});
    const auto Sigma = torch::mean(torch::matmul(difference.transpose(0, 1).view({particle_size_, 6, 1}),
                                                 difference.transpose(0, 1).view({particle_size_, 1, 6})), 0);
    const auto Sigma_cpu = Sigma.view({36}).to(torch::kCPU);
    auto Sigma_vector = std::vector<double>(Sigma_cpu.data_ptr<double>(), Sigma_cpu.data_ptr<double>() + 36);
    return Sigma_vector;
  }

  std::vector<double> SVGDICP::get_particles() const {
    const auto particle_pose_cpu = pose_particles_.to(torch::kCPU);
  ...
  std::vector<double> SVGDICP::get_particle_weight() {
    return std::vector(particle_size_, 1.0);
  }

  std::vector<std::vector<float> > SVGDICP::get_particle_history() const {
    const auto particle_stack_cpu = particle_stack_.to(torch::kCPU);
