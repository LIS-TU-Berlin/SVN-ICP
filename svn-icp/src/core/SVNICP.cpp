...
namespace svnicp {
  SVNICP::SVNICP(const SteinICPParam &parameters,
                 const torch::Tensor &init_pose,
                 const ParticleWeightOpt &opt): SVGDICP(parameters, init_pose.clone()) {
    ...
    pose_particles_ = torch::cat({t_.view({particle_size_, 3}).transpose(0, 1), rotm_to_ypr_tensor(R_).transpose(0, 1)},
                                 0);
  }


  SteinICPState SVNICP::stein_align() {
    static const auto early_stop_threshold = torch::tensor({config_.convergence_threshold}).to(torch::kCUDA);
    ...
    for (int epoch = 0; epoch < config_.iterations; epoch++) {
      ...
      // SO3-> so3
      pose_particles_ = torch::cat({
                                     t_.view({particle_size_, 3}).transpose(0, 1),
                                     rotm_to_ypr_tensor(R_).transpose(0, 1)
                                   }, 0);
      ...
      if (config_.check_early_stop) {
        if (torch::lt(stein_grad.norm(2, 1).mean(0),
                      early_stop_threshold).to(torch::kCPU).item<bool>()) {
  ...
  std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> SVNICP::Newton_grad_right(const torch::Tensor &source_paired,
    ...
    const auto s_hat = torch::stack({
                                      torch::stack({
                                                     ...
                                                     source_paired.index({Slice(), Slice(), 1})
                                                   }, 2),
                                      torch::stack({
                                                     source_paired.index({Slice(), Slice(), 2}),
                                                     zero,
  ...
  torch::Tensor SVNICP::to_rotation_tensor(const torch::Tensor &r, const torch::Tensor &p, const torch::Tensor &y) {
    static const auto zeros = torch::zeros({particle_size_}).to(torch::kCUDA).to(data_type);
    const auto angle_axis = torch::cat({r, p, y}, 1).view({particle_size_, 3});

    const auto angle = torch::norm(angle_axis, 2, 1).view({particle_size_, 1});
    const auto zero_index = torch::lt(angle, 1e-12).view({particle_size_, 1});
    const auto axis = torch::where(zero_index, torch::zeros({particle_size_, 3}).to(torch::kCUDA).to(data_type),
                                   angle_axis.div(angle));
    const auto cos_a = torch::cos(angle);
    const auto sin_a = torch::sin(angle);
    const auto a_hat = torch::stack({
  ...
  torch::Tensor SVNICP::rotm_to_ypr_tensor(const torch::Tensor &R) {
    const auto angle = torch::acos(
      torch::clip(
        0.5 * (R.index({Slice(), 0, 0}) + R.index({Slice(), 1, 1}) + R.index({Slice(), 2, 2}) - 1),
        -1, 1)
    ).view({particle_size_, 1});

    const auto sin_angle = torch::sin(angle);

    const auto nonzero_mask = (sin_angle.abs() > 1e-12).view({particle_size_, 1});
  ...
  torch::Tensor SVNICP::svgd_grad(const torch::Tensor &pose_parameters, const torch::Tensor &newton_grad,
  ...
  torch::Tensor SVNICP::svn_full_grad(const torch::Tensor &pose_parameters, const torch::Tensor &H,
    ...
    ////H_svn: [p, 6, 6]
    const auto grad2 = torch::matmul(grad.view({particle_size_, particle_size_, 6, 1}),
                                     grad.view({particle_size_, particle_size_, 1, 6})).sum(1);
    ...
    const auto update = config_.lr * torch::linalg::inv(H_mean).matmul(svgd_update).squeeze();
    return update;
  }

  std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> SVNICP::rbf_hessian_kernel(
    const torch::Tensor &x1, const torch::Tensor &H) {
    // || xi - xj ||
  ...
  void SVNICP::pose_update(const torch::Tensor &stein_grad) {
    const auto d_R = to_rotation_tensor(stein_grad.index({Slice(), 3}).reshape({particle_size_, 1, 1}),
  ...
  std::vector<double> SVNICP::get_particle_weight() {
    const auto weight_cpu = particle_weight_.to(torch::kCPU);
    return std::vector<double>(weight_cpu.data_ptr<float>(), weight_cpu.data_ptr<float>() + particle_size_);
  }

  torch::Tensor SVNICP::get_transformation() {
    //        return torch::mean(pose_particles_, 1);
    auto weighted_mean = torch::mul(pose_particles_, particle_weight_.transpose(0, 1)).sum(1);
    return weighted_mean;
  }

  torch::Tensor SVNICP::get_distribution() {
    const auto weight_mean = this->get_transformation();
    auto var = torch::mul((pose_particles_ - weight_mean.view({6, 1})).square(),
                          particle_weight_.transpose(0, 1)).sum(1);
    return var;
  }

  std::vector<double> SVNICP::get_cov_matrix() {
    const auto weight_mean = this->get_transformation();
    const auto difference = pose_particles_ - weight_mean.view({6, 1});
    const auto Sigma = torch::sum(particle_weight_.view({particle_size_, 1, 1})
