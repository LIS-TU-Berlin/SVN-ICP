...
namespace stein_icp {
    ...
    torch::Tensor initialize_particles(int particle_count, Device_type device, const torch::Tensor &ub, const torch::Tensor &lb) {
        torch::Tensor init_pose = torch::empty({6, particle_count, 1}, torch::TensorOptions().dtype(torch::kFloat64)).to(device);
        if (particle_count == 1)
    ...
    gtsam::Vector6 tensor2Vector6(const torch::Tensor &pose_tensor)
    {
        auto pose_tr_cpu = pose_tensor.to(torch::kCPU);
        std::vector<double> pose_vector = std::vector<double>(pose_tr_cpu.data_ptr<double>(), pose_tr_cpu.data_ptr<double>()+6);
...
//        auto rot_mat = rot.matrix();
    ...
    std::tuple<gtsam::Vector6, gtsam::Matrix66> left2right(std::vector<double> particles, const gtsam::Pose3 &initial_guess)
    {
        int particle_count = particles.size() / 6;
        Eigen::Matrix<double, 6, Eigen::Dynamic, Eigen::RowMajor> particle_matrix_tmp(6, particle_count);
        particle_matrix_tmp = Eigen::Map<Eigen::Matrix<double, 6, Eigen::Dynamic, Eigen::RowMajor>>(particles.data(), 6, particle_count);
        ...
        auto diff_matrix = particle_matrix_right - v_right;
